...
function basicAuth(req, res, next) {
  ...
  if (type === 'Basic' && token) {
    ...
    if (idx !== -1) {
      ...
      if (username === AUTH_USER && password === AUTH_PASS) return next();
    }
  }
  res.setHeader('WWW-Authenticate', 'Basic realm="QR Dashboard Login"');
...
function loadState(){
  ...
  syncState = { ...syncState, ...saved, running:false, stopRequested:false, currentMatricola:'', lastMessage:saved.lastMessage || 'Pronto.' };
}
function saveState(){ writeJson(STATE_PATH, syncState); }

function getScheduleConfig(){
  return readJson(SCHEDULE_PATH, {
    enabled:false, frequency:'daily', time:'01:00', dayOfWeek:'1', dayOfMonth:'1',
    batchSize:25, delayMs:1800, cacheHours:12, maxRetries:2
  });
}
function saveScheduleConfig(cfg){ writeJson(SCHEDULE_PATH, cfg); }

function shouldRunSchedule(config, now){
  if(!config.enabled) return false;
  const hh = String(now.getHours()).padStart(2,'0');
  const mm = String(now.getMinutes()).padStart(2,'0');
  if(`${hh}:${mm}` !== config.time) return false;
  if(config.frequency === 'daily') return true;
  if(config.frequency === 'weekly') return String(now.getDay()) === String(config.dayOfWeek);
  if(config.frequency === 'monthly') return String(now.getDate()) === String(config.dayOfMonth);
  return false;
}
...
async function initDb(){
  ...
  if(!fs.existsSync(DB_PATH)) throw new Error('Database non trovato: ' + DB_PATH);
...
function normalizeUrlInput(url){
  ...
  if(!value) throw new Error('Link QR mancante');
  if(!/^https?:\/\//i.test(value)) throw new Error('Il QR non contiene un link valido');
  return value;
}
function hasUsefulData(data){
  return Boolean(data && (data.matricola || data.ultima_trasmissione || data.stato || data.versione_software || data.denominazione));
}
function isCacheValid(entry, hours){
  if(!entry?.savedAt) return false;
  const age = Date.now() - new Date(entry.savedAt).getTime();
  return age >= 0 && age < Number(hours || 0) * 60 * 60 * 1000;
}

async function fetchAndParse(url, options = {}){
  const safeUrl = normalizeUrlInput(url);
  const cacheHours = Number(options.cacheHours ?? DEFAULT_SYNC_CONFIG.cacheHours);
  if(cacheHours > 0 && isCacheValid(cache[safeUrl], cacheHours)) {
    return { ...cache[safeUrl].payload, fromCache:true };
  }
  const response = await fetch(safeUrl, { headers:{ 'User-Agent':'Mozilla/5.0 QR Sync Locale' } });
  ...
  if(response.ok && hasUsefulData(payload.data)){
    cache[safeUrl] = { savedAt: nowIso(), payload };
    saveCache();
  }
  return payload;
}
...
function upsertImportedResult(url, data){
  ...
  const urlRows = all(`SELECT rowid AS id FROM urls WHERE url = $url LIMIT 1`, { $url:url });
  const urlId = urlRows[0]?.id || null;

  run(`
    INSERT INTO results (
      url_id, matricola, stato, ultima_vp, data_vp, risultato_vp, partita_iva_vp,
      cf_tecnico, ultima_trasmissione, versione_fw, partita_iva, denominazione,
      link_qr, last_sync, sync_error, sync_attempts
    )
    VALUES (
      $url_id, $matricola, $stato, $data_vp, $data_vp, $risultato_vp, $piva_laboratorio,
      $cf_tecnico, $ultima_trasmissione, $versione_software, $partita_iva, $denominazione,
      $url, $now, '', 0
    )
    ON CONFLICT(matricola) DO UPDATE SET
      url_id = COALESCE($url_id, url_id),
      stato = COALESCE(NULLIF($stato,''), stato),
      ultima_vp = COALESCE(NULLIF($data_vp,''), ultima_vp),
      data_vp = COALESCE(NULLIF($data_vp,''), data_vp),
      risultato_vp = COALESCE(NULLIF($risultato_vp,''), risultato_vp),
      partita_iva_vp = COALESCE(NULLIF($piva_laboratorio,''), partita_iva_vp),
      cf_tecnico = COALESCE(NULLIF($cf_tecnico,''), cf_tecnico),
      ultima_trasmissione = COALESCE(NULLIF($ultima_trasmissione,''), ultima_trasmissione),
      versione_fw = COALESCE(NULLIF($versione_software,''), versione_fw),
      partita_iva = COALESCE(NULLIF($partita_iva,''), partita_iva),
      denominazione = COALESCE(NULLIF($denominazione,''), denominazione),
      link_qr = $url,
      last_sync = $now,
      sync_error = '',
      sync_attempts = 0
  `, {
    $url_id:urlId, $matricola:matricola, $stato:data.stato || '', $data_vp:data.data_vp || '',
    $risultato_vp:data.risultato_vp || '', $piva_laboratorio:data.piva_laboratorio || '',
    $cf_tecnico:data.cf_tecnico || '', $ultima_trasmissione:data.ultima_trasmissione || '',
    $versione_software:data.versione_software || '', $partita_iva:data.partita_iva || '',
    $denominazione:data.denominazione || '', $url:url, $now:now
  });
  saveDb();
  return { matricola, url_id:urlId, last_sync:now };
}

function buildSyncQueue(mode='new'){
  const rows = all(`
    SELECT r.matricola, r.ultima_trasmissione AS prima_ultima_trasmissione, r.url_id,
           COALESCE(r.link_qr, u.url) AS url, r.last_sync, COALESCE(r.sync_attempts,0) AS sync_attempts
    FROM results r
    LEFT JOIN urls u ON u.rowid = r.url_id
    WHERE COALESCE(r.link_qr, u.url) IS NOT NULL AND COALESCE(r.link_qr, u.url) <> ''
    ORDER BY CASE WHEN r.last_sync IS NULL OR r.last_sync = '' THEN 0 ELSE 1 END,
             r.last_sync ASC, r.matricola ASC
  `);
...
function addSample(s){
  syncState.lastSamples.unshift(s);
...
async function syncOne(row, config){
  ...
  syncState.lastMessage = `Controllo ${syncState.checked + 1}/${syncState.total}: ${syncState.currentMatricola}`;
  saveState();

  let lastErr = null;
  for(let attempt=0; attempt <= Number(config.maxRetries || 0); attempt++){
    try{
      ...
      if(!hasUsefulData(data)){
        syncState.skipped++;
        run(`UPDATE results SET last_sync=$now, sync_error=$error, sync_attempts=COALESCE(sync_attempts,0)+1 WHERE matricola=$matricola`,
          { $now:nowIso(), $error:`Dati non trovati. HTTP ${statusCode}`, $matricola:row.matricola });
        saveDb();
        addSample({ matricola:row.matricola, esito:'saltato', http:statusCode, cache:fromCache });
        return;
      }

      const saved = upsertImportedResult(row.url, data);
      syncState.updated++;
      addSample({ matricola:saved.matricola, esito:'aggiornato', ultima_trasmissione:data.ultima_trasmissione || '', ultima_vp:data.data_vp || '', cliente:data.denominazione || '', cache:fromCache });
      return;
    }catch(err){
      lastErr = err;
      if(attempt < Number(config.maxRetries || 0)) await sleep(Math.max(1000, Number(config.delayMs || 1000)));
    }
  }

  syncState.errors++;
  run(`UPDATE results SET last_sync=$now, sync_error=$error, sync_attempts=COALESCE(sync_attempts,0)+1 WHERE matricola=$matricola`,
    { $now:nowIso(), $error:lastErr ? lastErr.message : 'Errore sconosciuto', $matricola:row.matricola });
  saveDb();
  addSample({ matricola:row.matricola, esito:'errore', errore:lastErr ? lastErr.message : 'Errore sconosciuto' });
}

async function processSyncQueue(mode='new', input={}){
  if(syncState.running) return syncState;
  const config = {
    ...DEFAULT_SYNC_CONFIG, ...input,
  ...
  let queue = buildSyncQueue(mode);
  syncState = {
    ...syncState, running:true, stopRequested:false, startedAt:nowIt(), finishedAt:null,
    ...
    currentMatricola:'', lastMessage:'Sync avviata', config,
...
app.get('/api/results', (req, res) => {
  ...
  const rows = all(`
    SELECT r.matricola, r.stato, COALESCE(r.data_vp, r.ultima_vp) AS data_vp,
           r.risultato_vp, r.partita_iva_vp AS piva_laboratorio, r.cf_tecnico,
           r.ultima_trasmissione, r.versione_fw AS versione_software, r.partita_iva,
           r.denominazione, COALESCE(r.link_qr, u.url) AS link_qr, r.last_sync, r.sync_error
    FROM results r
    LEFT JOIN urls u ON u.rowid = r.url_id
    WHERE (
      $q = '' OR r.matricola LIKE $like OR r.stato LIKE $like OR r.partita_iva LIKE $like OR
      r.partita_iva_vp LIKE $like OR r.denominazione LIKE $like OR COALESCE(r.link_qr, u.url) LIKE $like
    )
    AND ($vpMonth = '' OR substr(COALESCE(r.data_vp, r.ultima_vp, ''), 4, 2) = $vpMonth)
    AND ($vpYear = '' OR substr(COALESCE(r.data_vp, r.ultima_vp, ''), 7, 4) = $vpYear)
    ORDER BY r.matricola
    LIMIT $limit
  `, { $q:q, $like:like, $vpMonth: vpMonth === '00' ? '' : vpMonth, $vpYear:vpYear, $limit:limit });
  res.json(rows);
});

app.post('/api/import-url', async (req,res) => {
  try{
    const url = normalizeUrlInput(req.body && req.body.url);
    const { statusCode, data } = await fetchAndParse(url, { cacheHours:0 });
    const saved = upsertImportedResult(url, data);
...
app.post('/api/sync/start', (req,res) => {
  ...
  const mode = req.body && req.body.resume ? 'resume' : 'new';
  processSyncQueue(mode, req.body || {}).catch(err => {
    syncState.running=false; syncState.errors++; syncState.lastMessage='Errore sync: ' + err.message; syncState.finishedAt=nowIt(); saveState();
  });
...
app.post('/api/sync/stop', (req,res) => {
  ...
  syncState.lastMessage = 'Richiesta interruzione ricevuta. Finisco il record corrente e salvo.';
  saveState();
  res.json({ ok:true, state:publicSyncState() });
});
...
app.post('/api/schedule/save', (req,res) => {
  ...
  res.json({ ok:true, config:cfg, message:'Programmazione salvata' });
});
...
cron.schedule('* * * * *', async () => {
  try{
    ...
    const now = new Date();
    if(!shouldRunSchedule(cfg, now)) return;
    if(syncState.running) return;
    const runKey = `${now.toISOString().slice(0,16)}-${cfg.frequency}`;
    if(lastScheduleRunKey === runKey) return;
    lastScheduleRunKey = runKey;
    await processSyncQueue('new', cfg);
  }catch(err){ console.error('Errore scheduler:', err); }
});
